...
Protocol v3 also freezes the persistent parent-thread host identity, explicitly enables native multi-agent support, and binds the selected plan to persistent parent/child rollouts and their observed usage. `selected_mode` is a planning decision; `executed_mode` is populated only from host-backed execution evidence. A missing thread, unknown rollout schema, missing descendant usage, unrelated child, or declared-but-unobserved agent invalidates the experiment. Protocol v1 and the four attempted v2 preflight sessions remain invalid and are not reusable for v3 claims.
