...
Protocol v4 also freezes the persistent parent-thread host identity, explicitly enables native multi-agent support, and binds the selected plan to persistent parent/child rollouts and their observed usage. `selected_mode` is a planning decision; `executed_mode` is populated only from host-backed execution evidence. A missing thread, unknown rollout schema, missing descendant usage, unrelated child, declared-but-unobserved agent, or unbound lifecycle identifier invalidates the experiment. Protocols v1-v3 and their incomplete or invalid preflights are historical evidence only and are not reusable for v4 claims.
