...
description: Execute long or compaction-prone work with external state and evidence receipts. Use when the work will take a while and the thread is likely to be lost: resumable multi-turn work with several observable criteria that must pick up from where it stopped; abstain from focused single-pass tasks.
---
