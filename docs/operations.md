...
```powershell
& ./install.ps1 -ReleaseRef v1.10.0-rc.5
```

```bash
./install.sh --release-ref v1.10.0-rc.5
```
...
& $python scripts/verify_installed.py --source-root . `
  --installed-root <installed-root> --tag v1.10.0-rc.5
```
...
4. Nothing. `AUTO_PUBLISH` is `true`, so the green tag run dispatches
   ...
   fails -- v1.10.0-rc.5 shipped and was never recorded, so the documented rollback
