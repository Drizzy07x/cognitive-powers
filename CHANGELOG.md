...
- Closed a Unicode hole in parallel write ownership. The overlap check folded case but not composition, so two owners claiming the same file as `café.py` in composed and decomposed form were judged not to overlap and could be scheduled into the same wave. macOS and Windows resolve both spellings to one file. Owned paths are now composed before comparison.
