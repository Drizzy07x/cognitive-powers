...
Completeness is judged by Graphify's own incremental detector when the provider is importable, and otherwise from the manifest. The detector is asked about the AST layer, because `graphify update` is what the session-start refresh runs and its contract is AST-only: it leaves `semantic_hash` empty on purpose, so the detector's default semantic question re-queues every file that refresh just rebuilt and reports a maintained index as permanently pending. Semantic extraction of documents is a separate, provider-driven pass this plugin never performs, and a `complete` verdict here does not claim it is current. The reported `detector` names which one answered, and `completeness_verified` says whether coverage was actually established. The manifest detector reports `unverified` rather than `complete`: it can prove every indexed file is current, but it cannot see a file Graphify has never indexed, because Graphify's corpus rules are not derivable from its output. Results then carry `coverage_verified: false`, and absence from the candidate list is not evidence of absence. The session-start refresh is what closes that gap, so a long session that adds files should not assume the graph covers them.
