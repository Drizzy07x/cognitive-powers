...
- Select `focused`, `standard`, or `durable` with `<plugin-root>/scripts/orchestration_policy.py` when intensity is not obvious. Preserve its reasons and abstentions; do not infer permission to retrieve memory or delegate from intensity alone. The v1 intensity result remains separate from agent planning.
...
- After bounded discovery establishes non-trivial boundaries, obtain the compact v2 input with `<plugin-root>/scripts/orchestration_policy.py --agent-plan-template 2 --json`, fill it, and submit it with `--agent-plan <json-or-stdin> --json`. Copy every enum from `allowed_values` exactly: never invent aliases such as `write`, `workspace-write`, or `implement` for a field whose declared value is `change`. Do not add a `verifier` unit; provide the executable `verification_check` and let the planner create its fresh synthetic verifier. Never read the runtime source to discover the contract. Do not run the planner for an obvious focused edit.
...
- Execute the returned complete plan exactly: `solo`, `parallel-read-only`, `parallel-packets`, or `staged-verify`. Keep its assignment IDs immutable. After every wave, evaluate its stop conditions without generating a replacement plan; if it becomes invalid, stop delegation and report degradation. Interrupt duplicate or out-of-scope work as soon as it is observable.
- Spawn each assignment as the agent the host registered for its `role`, not as a general-purpose worker carrying the role in its prompt. The registered definition is where the tool set lives, and the tool set is what makes a read-only role read-only and depth one a property rather than a request. Read [routing.md](references/routing.md) for the registered names and the fallback when a host registers none, and record which one actually ran.
