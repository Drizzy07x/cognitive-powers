...
description: Select and apply the shortest reporting profile, compact, normal, or explicit, that still preserves evidence in a progress update, completion report, answer, or handoff. Compresses presentation, never evidence.
when_to_use: Use when the user asks for shorter or longer answers, for a compact technical handoff, for adaptive brevity, or for a set verbosity, when a handoff is consequential enough that profile choice is genuinely ambiguous, or when a reusable communication receipt is required.
---
