...
def main() -> int:
    ...
    assert (
        action["host"] == "chromium-public-surface"
        and action["visible"]["status"] == "Ready"
    )
    assert action["screenshotSha256"] == sha256(root / "final.png")
    assert (
        observer["host"] == "fresh-chromium-public-surface"
        and observer["visible"]["status"] == "Ready"
    )
    assert observer["events"] == EXPECTED and observer["actionReceiptSha256"] == sha256(
        root / "host-receipt.json"
    )
    print(
        json.dumps(
            {"passed": True, "observerSha256": sha256(root / "observer-receipt.json")}
        )
    )
    return 0
